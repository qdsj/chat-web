...
// 获取单聊 - 聊天记录
export const getSingleChatHistoryApi = (data: { roomId: string }): Promise<I_GetSingleChatHistoryResultApi> => {
	return postFetch("/chat/getSingleChatHistory", data);
};

// 获取群-聊天记录
export const getGroupChatHistoryApi = (data: { roomId: string }): Promise<I_GetGroupChatHistoryResultApi> => {
...
// 更新聊天窗口的时间
export const updateChatWindowsTimeApi = (data: { roomId: string; type: ConversationType }) => {
...
// 增加一个会话记录
export const addSessionApi = (data: { roomId: string; type: ConversationType }) => {
	return postFetch("/session/add", data);
};

// 删除一个会话记录
export const deleteSessionApi = (data: { roomId: string; type: ConversationType }) => {
