...
export const useChatStore = defineStore(
	...
	() => {
		...
		const getChatHistoryByRoomId = async (roomId: string) => {
			const targetGroup = conversationsList.value.find((obj) => obj.id === roomId);
			if (targetGroup && targetGroup.messages.length == 0) {
				const fetchHistory = targetGroup.type === "group" ? getGroupChatHistory : getSingleChatHistory;
		...
		const createMessage = (params: Omit<Message, "id" | "createdAt">): Message => {
			const { senderId, roomId = currentConversation.value!.id, content, msgType = "text" } = params;
			return {
				id: uuidv4(),
				roomId,
				senderId,
		...
		const addMessage = async (message: SocketMessage) => {
			...
			if (!isSenderInConversations) {
				addConversation({
					id: message.roomId,
					name: await getChatNameByRoomId({ roomId: message.roomId, type: message.type }),
					avatar: "",
					messages: [_message],
					type: message.type,
				});
