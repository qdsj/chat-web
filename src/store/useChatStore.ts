...
import { Conversation, ConversationType, Message, MsgType, SocketMessage } from "@/types/model/chat.type";
import {
	addSessionApi,
	deleteSessionApi,
...
export const useChatStore = defineStore(
	...
	() => {
		...
		const getChatHistoryByRoomId = async (roomId: string, type: ConversationType) => {
			const fetchHistory = type === "group" ? getGroupChatHistory : getSingleChatHistory;
		...
		const handleConversation = async (params: {
			...
			try {
				const hasConversation = conversationsList.value.some((item) => item.id === params.id);
				// 如果不存在，则添加到会话列表中
				if (!hasConversation) {
					await addSession(params.id, params.type || "person");
					const messages = await getChatHistoryByRoomId(params.id, params.type || "person");
					addConversation({
						id: params.id,
						name: params.name,
		...
		const createMessage = (params: Omit<Message, "id" | "createdAt">): Message => {
			const { senderId, roomId = currentConversation.value!.id, content, msgType } = params;
			return {
				id: uuidv4(),
				roomId,
				senderId,
		...
		const addMessage = async (message: SocketMessage) => {
			if (message.type == "server") {
				if (message.msgType === "new-message") {
			...
			// 接收消息的人是否在会话列表中
			const isSenderInConversations = conversationsList.value.some((conv) => conv.id === message.roomId);
			...
			if (!isSenderInConversations) {
				...
				if (message.type === "group") {
					const [_, res] = await groupStore.getGroupMemberCountByList(message.roomId, message.type);
					memberCount = res;
				}
			...
			const targetSession = conversationsList.value.find((session) => session.id === message.roomId);
			if (
				targetSession &&
		...
		const getChatInfoByRoomId = async (params: { roomId: string; type: string }) => {
		...
		const getSessionList = async () => {
			...
			conversationsList.value = sessionList.data.map((item) => {
				return {
					id: item.roomId,
					name: item.type === "group" ? item.roomInfo.name : item.roomInfo.username || "",
			...
			for (let i = 0; i < conversationsList.value.length; i++) {
				...
				const messages = await getChatHistoryByRoomId(item.id, item.type as ConversationType);
				...
				if (currentConversation.value?.id && item.id !== currentConversation.value?.id) {
		...
		// 删除会话记录
		const deleteSession = async (data: { roomId: string; type: ConversationType }) => {
			try {
