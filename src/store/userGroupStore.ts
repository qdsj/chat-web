...
export const useGroupStore = defineStore(
	...
	() => {
		...
		const createGroupChat = async (userId: string[]): Promise<I_CreateGroupChatApiResult["data"]> => {
		...
		const updateGroupChatInfo = async (roomId: string, description: string, type: string) => {
			try {
		...
		const getGroupMemberByList = async (
			roomId: string,
			type: string
		): Promise<[string | null, I_GetGroupMemberInfoApiResult["data"] | null]> => {
			const group = groupList.value.find((item) => item.id === roomId && item.type === "group");
			if (!group) {
		...
		const getGroupMemberCountByList = async (
			roomId: string,
			type: string
		): Promise<[string | null, I_GetGroupMemberCountApiResult["data"] | null]> => {
			const targetGroup = groupList.value.find((obj) => obj.id === roomId && obj.type === type);
			if (!targetGroup) {
				return ["targetGroup is null", null] as any;
			}
