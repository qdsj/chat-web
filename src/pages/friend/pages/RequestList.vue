      ...
      <div class="apply-item"
        ...
        <div class="op-btn">
          <div v-if="item.friendShip.status == 'pending'">
            <div v-if="item.id !== item.friendShip.requesterId"
                 class="result-name">
              {{ "等待对方同意好友申请" }}
            </div>
            <div v-else>
              <el-dropdown placement="bottom-end"
                ...
                <span class="el-dropdown-link">
                  <el-button type="primary"
                             @click="agreeApply(item.id, item.username)">接受</el-button>
                </span>
              </el-dropdown>
            </div>
          </div>
          <div v-else
               class="result-name">{{ item.friendShip.status }}</div>
        </div>
      </div>
    </div>
      ...
      <el-form :model="formData"
        ...
        <el-form-item label="备注名"
          ...
          <el-input v-model="formData.remarkName"
                    placeholder="请输入备注名"></el-input>
        </el-form-item>
      </el-form>
    </Dialog>
    <div v-if="!applyList"
         class="no-data">暂无申请</div>
  </ContentPanel>
</template>
