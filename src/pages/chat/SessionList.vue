  ...
  const sessionList = computed
    (() => {
      if (chatStore.isSearching) {
        return chatStore.searchSessionList
      } else {
        return chatStore.conversationsList;
      }
    });
...
<template>
  <!-- 会话列表循环 -->
  <div v-for="item in sessionList"
       :key="item.id">
    <div @contextmenu.prevent="(e) => handleRightClick(e, item)"
         :class="{ active: chatStore.currentConversation?.id === item.id }">
      <ChatSession :data="item"
                   :avatar="item.avatar"
  ...
  <!-- 全局右键菜单（仅一个实例） -->
  <ContextMenu :menu-items="currentMenuItems"
